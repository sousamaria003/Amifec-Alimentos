...
const PERGUNTAS = [
    ...
    {
        q: 'Qual a nota média da Amifec nas avaliações do Google?',
        op: ['3.5/5', '4.0/5', '4.5/5', '4.8/5'],
        correta: 3,
        exp: 'A Amifec mantém nota 4.8/5 no Google, com 47 avaliações de clientes.'
    },
    {
...
/* ============================================
   GALERIA CLICÁVEL (MODAL)
   ============================================ */
(function () {
    const modal = document.getElementById('galModal');
    if (!modal) return;
    // Move o modal para o <body> para escapar de qualquer ancestral com `transform`
    // (sections com reveal/animações criam containing block e quebram `position: fixed`).
    if (modal.parentElement !== document.body) {
        document.body.appendChild(modal);
    }
    const imgEl = document.getElementById('galModalImg');
    const nomeEl = document.getElementById('galModalNome');
    const iconeEl = document.getElementById('galModalIcone');
    const infoEl = document.getElementById('galModalInfo');
    const closeBtn = document.getElementById('galModalClose');

    document.querySelectorAll('.galeria-card').forEach(card => {
        card.addEventListener('click', () => {
            const titulo = card.dataset.titulo || card.querySelector('p')?.textContent || '';
            const icone = card.dataset.icone || '🖼️';
            const info = card.dataset.info || 'Sem informações adicionais.';
            ...
            // Trava o scroll preservando a posição atual (modal aparece sobre a tela onde o usuário está)
            const sy = window.scrollY || window.pageYOffset || 0;
            document.body.dataset.scrollLock = String(sy);
    ...
    function fechar() {
        ...
        const sy = parseInt(document.body.dataset.scrollLock || '0', 10);
        ...
        // Restaura a posição instantaneamente (sem smooth-scroll)
